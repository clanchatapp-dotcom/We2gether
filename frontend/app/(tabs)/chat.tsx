...
export default function Chat() {
  ...
  // Status line (iMessage/WhatsApp-style) sits under the most recent of MY
  // messages: "Sent" → "Delivered" (partner's device fetched it) → "Read HH:mm"
  // (partner actually opened the chat). Only the newest of my messages shows it.
  ...
  const pickAndSend = useCallback(
    async (mediaType: "image" | "video") => {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        // No re-encoding on library picks — this preserves animated GIFs (a
        // quality value flattens them to a still frame) and original videos.
        videoMaxDuration: 60,
      });
  ...
  const renderItem = ({ item }: { item: Msg }) => {
    ...
    const showStatus = mine && item.id === lastMineId;
    let statusText: string | null = null;
    if (showStatus) {
      if (item.read_at) statusText = `Read ${formatReadTime(item.read_at)}`;
      else if (item.delivered_at) statusText = "Delivered";
      else statusText = "Sent";
    }
    const readLabel = statusText ? (
