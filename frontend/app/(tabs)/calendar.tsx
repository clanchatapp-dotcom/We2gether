...
import { ukTodayStr, addDaysStr, labelParts, dayLabelUK } from "@/src/uk-date";
...
function dayLabel(dateStr: string) {
  return dayLabelUK(dateStr);
}

// A cross-platform date strip anchored to UK time: today + next 89 days, so
// both partners (UK + Moscow) see the exact same "today" regardless of the
// phone's own timezone. Events auto-expire in the past anyway.
const UK_TODAY = ukTodayStr();
const DATE_OPTIONS = Array.from({ length: 90 }, (_, i) => {
  const value = addDaysStr(UK_TODAY, i);
  const p = labelParts(value);
  return { value, label: p.dow, day: p.day, mon: p.mon };
});
